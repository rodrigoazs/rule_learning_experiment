...
consts = {'person':set(),'faculty':set(),'course':set(), 'title':set()}
types = {
...
color = {'person': 'green', 'faculty': 'red', 'course': 'yellow', 'title': 'purple'}

#dataset = []
with open('../slipcover/others/uwcselearn.pl') as f:
    for line in f:
        ...
        if m:
            ...
            if relation in types:
                ...
                entity = entities[1]
                value = entities[2] if len(entities) > 2 else entities[1]
...
# train model
model = Word2Vec(sentences, min_count=1, sg=1, workers=4)
